...
def get_workflow_action_url(action, doc, user):
    apply_action_method = "/api/method/vcmrentals.rentals.doctype.ride_order.ridealm.rideworkflow_action"
    params = {
...
def get_approval_link(doc, user, allowed_options):
    #logging.debug(f"in get_approval_link PREQ {doc},{user} {allowed_options}")
    if "L1 Approve" in allowed_options:
        return get_workflow_action_url(action="L1 Approve", doc=doc, user=user)
    if "L2 Approve" in allowed_options:
        return get_workflow_action_url(action="L2 Approve", doc=doc, user=user)
    if "L3 Approve" in allowed_options:
        return get_workflow_action_url(action="L3 Approve", doc=doc, user=user)
    if "Final Approve" in allowed_options:  # Final Approve
